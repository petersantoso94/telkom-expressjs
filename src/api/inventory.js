...
router.post('/return', sessionMiddleware, (req, res) => {
  try {
    ...
    MoveInventory(req, res, 'Return', (stat) => stat.status == Status.Shipout || stat.status == Status.Consignment, Status.Return)
  } catch (error) {
    console.error(
      `POST return inventories >> ${error.stack})`
    );
    res.status(500).json();
  }
})
...
router.post('/shipout', sessionMiddleware, (req, res) => {
  try {
    ...
    MoveInventory(req, res, 'Shipout', (stat) => stat.status == Status.Shipin || stat.status == Status.WarehouseMovement || stat.status == Status.Return, Status.Shipout)
  } catch (error) {
    console.error(
...
const MoveInventory = (req, res, status_string, required_status_condition, status) => {
  ...
  req.body.forEach(({
    ...
    if (!Serial_Number || !Price || !Warehouse || !Remark_Inv || !Form_Number || !Fabiao_Number) {
      res.status(400).json({
        message: 'Wrong inventory object fields, required (Serial_Number, Price, Warehouse, Remark_Inv, Form_Number, Fabiao_Number)'
      });
      return;
    }
    // make inventory_movement object
  ...
  Inventory.getStatusFromSerialNumbers(list_serial_number).then(async (listStatus) => {
    ...
    InventoryMovement.create(inv_move, req.userId, status).then(async () => {
      //change the status on inventory
      Inventory.updateStatus(list_serial_number, req.userId, status).then(() => {
        res.status(201).json();
        return;
      }).catch(err => {
        console.error(
